...
async def call_llm(
    ...
    **overrides
    ...
    # Count input tokens
    input_tokens = TokenCounter.count_tokens_in_langchain_messages(langchain_messages, model)
    
    # Check usage limits if user_id is provided
    if user_id:
        # Estimate cost for this request
        max_tokens = config.get("max_tokens", 4096)
        estimated_output_tokens = TokenCounter.estimate_response_tokens(model, max_tokens)
        estimated_cost = UsageService.calculate_cost(model, input_tokens, estimated_output_tokens)
    ...
    try:
        ...
        if user_id and full_response:
            ...
            print(f"Recorded usage: {input_tokens} input + {output_tokens} output tokens for user {user_id}")

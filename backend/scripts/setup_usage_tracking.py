...
def main():
    ...
    parser = argparse.ArgumentParser(description="Set up usage tracking database schema")
    parser.add_argument(
        "--check", 
        action="store_true", 
    ...
    parser.add_argument(
        "--drop", 
        action="store_true", 
        help="Drop existing schema (use with caution)"
    )
    parser.add_argument(
        "--clear-data", 
        action="store_true", 
    ...
    try:
        ...
        if args.check:
            ...
            confirm = input("Are you sure? This will remove all constraints and indexes. (y/N): ")
            if confirm.lower() == 'y':
                drop_usage_tracking_schema()
            ...
            confirm = input("Are you absolutely sure? This cannot be undone. Type 'DELETE' to confirm: ")
            if confirm == 'DELETE':
                clear_usage_data()

...
from backend.models.schemas import UsageSummary, SetUsageLimitRequest, UsageHistoryRequest
...
async def set_user_usage_limit(
    ...
    try:
        if request.monthly_limit < Decimal("0"):
            raise HTTPException(status_code=400, detail="Monthly limit must be non-negative")
...
async def get_usage_stats(
    ...
    try:
        ...
        stats = {
            ...
            "total_cost": float(result[0]["total_cost"]) if result and result[0]["total_cost"] else 0,
            "active_users": result[0]["active_users"] if result else 0,
            "avg_cost_per_request": float(result[0]["avg_cost_per_request"]) if result and result[0]["avg_cost_per_request"] else 0,
            "monthly_requests": monthly_result[0]["monthly_requests"] if monthly_result else 0,
            "monthly_cost": float(monthly_result[0]["monthly_cost"]) if monthly_result and monthly_result[0]["monthly_cost"] else 0,
            "monthly_active_users": monthly_result[0]["monthly_active_users"] if monthly_result else 0,
        }
    ...
    try:
        ...
        for record in result:
            ...
            user_limits.append({
                ...
                "usage_percentage": float((current_usage / Decimal(str(record["monthly_limit"]))) * 100) if record["monthly_limit"] > 0 else 0,
